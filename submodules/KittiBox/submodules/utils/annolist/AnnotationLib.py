...
from incl.utils.annolist import AnnoList_pb2
from incl.utils.annolist import PalLib
...
class AnnoRect(object):
        def __init__(self, x1=-1, y1=-1, x2=-1, y2=-1, height=-1, width=-1, length=-1, x=-1, y=-1, z=-1, alpha=-1,
                     calib=None, calib_pinv=None):

                self.x1 = x1
                self.y1 = y1
                self.x2 = x2
                self.y2 = y2
                self.height_3d = height
                self.width_3d = width
                self.length_3d = length
                self.x_3d = x
                self.y_3d = y
                self.z_3d = z
                self.alpha = alpha
                self.calib = calib
                self.calib_pinv = calib_pinv
                self.z_3d_scale = 1.0          
        ...
        def bottom2center(self):
            self.y_3d -= self.height_3d*0.5
            return
